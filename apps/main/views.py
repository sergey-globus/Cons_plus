...
# шаблоны в генераторре документов
from django.shortcuts import render, get_object_or_404
...
def document_result(request):
    if request.method == 'POST':
        ...
        document = template.template_content  # шаблон с метками

        context = {
            'template_name': template.name,
            'document': document,
            'is_template_preview': True,  # Устанавливаем флаг для preview
        }
        return render(request, 'main/document_result.html', context)
...
def reference_materials(request):
    query = request.GET.get('q', '').strip()  # убираем пробелы по краям
